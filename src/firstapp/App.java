...
public class App {
	...
	private void run() {
		...
		if (condition(denominator)) {
			...
			System.out.println("A sorozat " + select(denominator) + ". elem az!");
		...
		System.out.println(this);
		System.out.println("Egyszerü kiválasztásos rendezéssel:");
	}
	...
	private boolean condition(final int denaminator) {
		int i = 0;
		while (i < list.size() && list.get(i) % denaminator != 0) {
			i = i + 1;
		}
		return i < list.size();
	}
	...
	private int find(final int denaminator) {
		int i = 0;
		while (i < list.size() && list.get(i) % denaminator != 0) {
			i = i + 1;
		}
